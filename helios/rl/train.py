...
from helios.rl.components.sampler import Sampler
...
class RLTrainer:
    ...
    def __init__(self, args):
        ...
        # build sampler
        self.sampler = self._hp.sampler(self.conf.sampler, self.env, self.agent, self.logger, self._hp.max_rollout_len) # HierarchicalSampler
        ...
        if args.mode == 'train':
            ...
            ## -- render videos and save it to ./data folder --
            # self.val(render=True, mode='rgb_array')
        elif args.mode == 'val':
            self.val(render=True, mode='rgb_array')
        else:
    ...
    def val(self, render:bool=False, mode:str='rgb_array'):
        """Evaluate agent."""
            ...
            with torch.no_grad():
                with timing("Eval rollout time: "):
                    for _ in range(WandBLogger.N_LOGGED_SAMPLES):   # for efficiency instead of self.args.n_val_samples
                        val_rollout_storage.append(self.sampler.sample_episode(is_train=False, render=render, mode=mode))
        ...
        if self.is_chef:
            with timing("Eval log time: "):
                self.agent.log_outputs(rollout_stats, val_rollout_storage,
                                       self.logger, log_images=True if (render is True and mode in ['rgb_array']) else False, step=self.global_step)
    ...
    def resume(self, ckpt, path=None, weights_folder:str=None):
        if weights_folder is None:
            path = os.path.join(self._hp.exp_path, 'weights') if path is None else os.path.join(path, 'weights')
        else:
            path = os.path.join(self._hp.exp_path, weights_folder) if path is None else os.path.join(path, weights_folder)
        assert ckpt is not None 
        # -- Updating DPMM parameters for HRL experiments --
        weights_file = CheckpointHandler.get_resume_ckpt_file(ckpt, path) # loading from path
...
if __name__ == '__main__':
    ...
    # Check if the 'experiments' directory exists, if not, create it
    ...
    print(f"EXP_DIR: {os.getenv('EXP_DIR')}")
