...
from helios.utils.general_utils import AttrDict, RecursiveAverageMeter, ParamDict
...
class ReplayBuffer:
    ...
    def __init__(self, hp):
        ...
        self._max_capacity = int(self._hp.capacity)
    ...
    def append(self, experience_batch):
        ...
        idxs = np.arange(self._idx, self._idx + n_samples) % self._max_capacity
        ...
        self._idx = (self._idx + n_samples) % self._max_capacity
        self._size = min(self._size + n_samples, self._max_capacity)
    ...
    def _init(self, example_batch):
        ...
        for key in example_batch:
            ...
            self._replay_buffer[key] = np.empty([self._max_capacity] + list(example_element.shape),
                                                dtype=example_element.dtype)
        self._idx = 0
        self._size = 0
...
class UniformReplayBuffer(ReplayBuffer): # New type of buffer for spirl-dpmm
    ...
    def sample(self, n_samples, filter=None):
        ...
        idxs = np.random.randint(0, self.size, size=n_samples)
...
# class UniformReplayBuffer(ReplayBuffer):# Old Uniform replay buffer for Spirl-DPMM
#     """Samples batch uniformly from all experience samples in the buffer."""
#     def sample(self, n_samples, filter=None):
#         assert n_samples <= self.size      # need enough samples in replay buffer
