...
from helios.utils.general_utils import ParamDict, get_clipped_optimizer, AttrDict, prefix_dict, map_dict, \
                                        nan_hook, np2obj, ConstantSchedule
from helios.utils.pytorch_utils import RAdam, remove_grads, map2np, map2torch
from helios.utils.vis_utils import add_caption_to_img, add_captions_to_seq
from helios.rl.components.normalization import DummyNormalizer
from helios.rl.components.policy import Policy
from helios.components.checkpointer import CheckpointHandler
...
class BaseAgent(nn.Module):
    ...
    def _get_optimizer(self, optimizer, model, lr):
        ...
        if optimizer == 'adam':
            ...
            get_optim = partial(get_clipped_optimizer, optimizer_type=AdamW, betas=(self._hp.adam_beta, 0.999))
...
class HierarchicalAgent(BaseAgent):
    """Implements a basic hierarchical agent with high-level and low-level policy/policies.
        For Spirl DPMM model:
        High level agent: ActionPriorSACAgent
        Low-level agent: SACAgent    
    """
    def __init__(self, config):
        super().__init__(config)
        self.hl_agent = self._hp.hl_agent(self._hp.overwrite(self._hp.hl_agent_params)) # For Spirl DPMM: ActionPriorSACAgent
        self.ll_agent = self._hp.ll_agent(self._hp.overwrite(self._hp.ll_agent_params)) # Dor Spirl DPMM: SACAgent
    ...
    def act(self, obs):
        ...
        # Tracking the original component responsiblity for this action (DPMM-dependency)
        ...
        # TODO: add tracking recording functionality
...
# -- Spirl DPMM agent -- RL for long-horizon --
class FixedIntervalHierarchicalAgent(HierarchicalAgent):
    ...
    def log_outputs(self, logging_stats, rollout_storage, logger, log_images, step):
        """Additionally provides option ot visualize hierarchical agents.
            rollout from rollout_storage.rollouts:
                dict_keys(['observation', 'info', 'observation_next', 'done', 'reward', 'image', 'agent_output', 'action'])
            for agent_output: 
                dict_keys(['is_hl_step', 'hl_action', 'hl_log_prob', 'hl_dist', 'action', 'log_prob', 'dist', 'dpmm_hard_assignment'])


        """
        try:
            super().log_outputs(logging_stats, rollout_storage, logger, False, step)
        except:
            pass
        if log_images:
            import imageio
            ...
            import datetime
            print('-- recording rendering file -- ')
            data_path = os.path.join(os.environ['DATA_DIR'], f'{datetime.datetime.now().strftime("_%Y-%m-%d_%H-%M-%S")}')
            if not os.path.exists(data_path):
                os.makedirs(data_path)

            for idx, rollout in enumerate(rollout_storage.rollouts):
                if 'image' in rollout.keys():
                    imgs = [np.array(to_pil_image(img)) for img in rollout.image]
                    imageio.mimsave(f'{data_path}/rendering_{idx}.mp4', imgs)
                # save statistic data
                npz_data = {}
                npz_data['obs'] = rollout['observation']
                npz_data['obs_next'] = rollout['observation_next']
                npz_data['done'] = rollout['done']
                npz_data['reward'] = rollout['reward']
                goal = []
                for i in rollout['info']:
                    goal.append(i[0]['obs_dict']['goal'])
                npz_data['goal'] = goal
                is_hl_step = []
                hl_action = []
                action = []
                dpmm_assignment = []
                for data_dict in rollout.agent_output:
                    is_hl_step.append(data_dict['is_hl_step'])
                    hl_action.append(data_dict['hl_action'])
                    action.append(data_dict['action'])
                    dpmm_assignment.append(data_dict['dpmm_hard_assignment'])
                npz_data['is_hl_step'] = is_hl_step
                npz_data['hl_action'] = hl_action
                npz_data['action'] = action
                npz_data['dpmm_assignment'] = dpmm_assignment
                npz_data['tasks_to_complete'] = rollout['info'][0][0]['tasks_to_complete']
                np.savez(f'{data_path}/rollout_data_{idx}.npz', **npz_data)
                # print('rollout info keys', rollout['info'][0])
                
            print(f'finished recording, saved data to {data_path}')
