...
from helios.utils.general_utils import listdict2dictlist, AttrDict, ParamDict, obj2np
...
class Sampler:
    ...
    def sample_action(self, obs):
        return self._agent.act(obs) # agent_output -> dpmm_hard_assignment
    ...
    def sample_episode(self, is_train, render=False, mode='rgb_array'):
            ...
            with self._agent.val_mode() if not is_train else contextlib.suppress():
                with self._agent.rollout_mode():
                    while not done and self._episode_step < self._max_episode_len:
                        # perform one rollout step
                        agent_output = self.sample_action(self._obs) # agent_output -> dpmm_hard_assignment
                        if agent_output.action is None:
                            break
                        agent_output = self._postprocess_agent_output(agent_output)
                        if render:
                            render_obs = self._env.render(mode=mode) # test render "change to human can direct render"
                        obs, reward, done, info = self._env.step(agent_output.action)
                        obs = self._postprocess_obs(obs)
                        ...
                        if render and mode in ['rgb_array']:
